    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if epoch % self.valid_sampling_in_n_epoch == 0:
            ...
            f0_gtpd_image = generate_graph_overwrite(vector1=f0_gt,
                                                     ...
                                                     x_label = 'Frames',
                                                     y_label = "Hz")
            self.logger.experiment.add_image("val/f0_pd", torch.from_numpy(f0_pd_image).clone().permute(2, 0, 1), self.current_epoch)
            self.logger.experiment.add_image("val/f0_gt-pd", torch.from_numpy(f0_gtpd_image).clone().permute(2, 0, 1), self.current_epoch)
