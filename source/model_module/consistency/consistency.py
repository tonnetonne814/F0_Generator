...
class Consistency(LightningModule):
    def __init__(
        ...
        *,
        ...
        data_std: float = 0.5, # 要変更 ref https://arxiv.org/pdf/2206.00364
        time_min: float = 0.002, # 変更NG ref https://arxiv.org/pdf/2206.00364
        time_max: float = 80.0,  # 変更NG ref https://arxiv.org/pdf/2206.00364
        bins_min: int = 2,      # EMA計算時のサンプル最小数
        bins_max: int = 150,    # EMA計算時のサンプル最大数
        bins_rho: float = 7,    # Karras et al formula(2022)より
        ...
        **kwargs,
        ...
        self.model = model
        self.model_ema = copy.deepcopy(model) # Consistency Training用モデル複製
    ...
    def training_step(self, images: torch.Tensor, *args, **kwargs):
        ...
        noise = torch.randn(images.shape, device=images.device) # ノイズ生成t=T
        timesteps = torch.randint(
            0,
            _bins - 1, # bins数以下まで
            (images.shape[0],), # =Batch個
        ...
        loss = self.loss_fn(self(next_noise_image, next_times), target) # CT Loss
