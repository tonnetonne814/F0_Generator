...
from source.model_module.consistency.consistency_utils import pad_dims_like
...
class ConsistencyTraining:
    ...
    def __init__(
        ...
        self.sigma_min = hps["Consistency"]["sigma_min"]
        self.sigma_max = hps["Consistency"]["sigma_max"]
        self.rho = hps["Consistency"]["rho"]
        self.sigma_data = hps["Consistency"]["sigma_data"]
        self.initial_timesteps = hps["Consistency"]["initial_timesteps"]
        self.final_timesteps = hps["Consistency"]["final_timesteps"]
        ## add ##
        self.initial_ema_decay_rate = hps["Consistency"]["initial_ema_decay_rate"]
        self.student_model_ema_decay_rate = hps["Consistency"]["student_model_ema_decay_rate"]
        self.use_improved_consistency = hps["Consistency"]["use_improved_consistency"]
    def __call__(
        self,
    ...
    def calc_loss(self, outputs):
        ...
        L2_loss = torch.nn.functional.mse_loss(
            outputs.predicted.clamp(-1.0, 1.0), outputs.target.clamp(-1.0, 1.0)
        )
        overflow_loss = torch.nn.functional.mse_loss(
            outputs.predicted, outputs.predicted.detach().clamp(-1.0, 1.0)
        )

        loss = L2_loss + overflow_loss
        return {"all_loss": loss,
                "L2_CT_loss": L2_loss,
                "overflow_loss": overflow_loss,
                "num_timesteps": outputs.num_timesteps,}
...
class ImprovedConsistencyTraining:
    ...
    def __init__(
        ...
        self.initial_timesteps = hps["Consistency"]["initial_timesteps"]
        self.final_timesteps = hps["Consistency"]["final_timesteps"]
        self.lognormal_mean = hps["Consistency"]["lognormal_mean"]
        self.lognormal_std = hps["Consistency"]["lognormal_std"]
        ## add ##
        self.initial_ema_decay_rate = hps["Consistency"]["initial_ema_decay_rate"]
        self.student_model_ema_decay_rate = hps["Consistency"]["student_model_ema_decay_rate"]
        self.use_improved_consistency = hps["Consistency"]["use_improved_consistency"]
    ...
    def calc_loss(self, outputs):
        ...
        #)
        huber_loss = (
            pseudo_huber_loss(outputs.predicted, outputs.target) * outputs.loss_weights
        ).mean()

        loss = huber_loss# + overflow_loss
        return {"all_loss": loss,
                "pseudo_huber_loss": huber_loss,
                #"overflow_loss": overflow_loss,
                "num_timesteps": outputs.num_timesteps,}
...
if __name__ == "__main__":
    ...
    noise = torch.randn(size=(3,1,423))
    sigma=80
    x = noise + sigma * torch.randn_like(noise) if add_initial_noise else noise
    steps = 80.0
    print(skip_scaling(sigma=torch.tensor(steps)))
    print(output_scaling(sigma=torch.tensor(steps)))