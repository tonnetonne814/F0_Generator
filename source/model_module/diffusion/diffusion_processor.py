...
from source.model_module.diffusion.diffusion_utils import  compute_hyperparams_given_schedule, std_normal, map_noise_scale_to_time_step

class DiffusionProcessor(nn.Module):
    def __init__(self, hps ):
        super(DiffusionProcessor, self).__init__()
    ...
    def get_beta(self, noise_schedule, timesteps=None, training=True):
        ...
        if noise_schedule == "cosine_beta":
            ...
            beta = torch.linspace(beta_0, beta_T,timesteps)
    ...
    def get_noisy_f0(self, f0, ts):
        ...
        if device.type == "cpu":
          pass
        noise_gt = std_normal(f0.shape).to(f0.device)
        ts=ts.view(f0.size(0), 1, 1)
        self.alpha = self.alpha.to(f0.device)
    ...
    def get_noise_schedule(self, timesteps=None, training=False):
        ...
        if self.hps["Diffusion"]['noise_schedule'] != '':
            ...
            if isinstance(noise_schedule, list):
                beta = torch.FloatTensor(noise_schedule)
            else:
        ...
        # Select Schedule
            ...
            if reverse_step == 1000:
                beta = torch.linspace(0.000001, 0.01, 1000)
    ...
    def denoising(self, x, noise_pd, ddim, n):
        ...
        if ddim:
            ...
            if n > 0:
                x = x + self.sigma_infer[n] * self.generate_x(self.size).to(x.device)
            else:
                #print()
                pass
        return x
